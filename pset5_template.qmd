...
earliest date we want to scrape. This loop will be 
<<<<<<< HEAD
part of the year_scraper() function. I realized
that a regular loop would not work as I 
was building the actual code.
=======
part of the year_scraper() function.
>>>>>>> eb89a441ced676097d3c4afdd476adffbbc6f088
...
<<<<<<< HEAD
  #then run for loop 
  if year_month <= 2013:
      print ("Turn back! Years 2013 and later, only!")
  else:
      while date_tag <= year_month:
          run function for assembling dataframe containing:
            Enforcement Title, Date, Category, Link, and Agency
            #for each cycle, append the results to the 
            #receiving object
            enforcement_actions_year_month.append(dataframe)
            #will cause the scraper to delay for 1 second
            time.sleep(1) 
  #return the fully stocked dataframe and save it to .csv       
=======
  run function for assembling dataframe containing
...
Step 2 B Answer:
The output of this code was originally 1,540 entries, due
to a small error that makes it so that a few entries
with dates earlier than January 1st, 2023 slip through. 
After trimming the (6) entries from 2022, we are left with 
1,534 entries, or 1,534 enforcement actions. The earliest entry of these trimmed entries is
January 3rd, 2023. In this case, a podiatrist 
pays $90,000 to settle false billing allegations. It is under the category of 
Criminal and Civil Actions, and the 
associated agency is the U.S. Attorney's 
Office.
...
<<<<<<< HEAD
enforcement_actions_2023["Observations"] = one_dummy(len(enforcement_actions_2023))
=======
df["Observation"] = one_dummy(len(df))
>>>>>>> eb89a441ced676097d3c4afdd476adffbbc6f088
...
<<<<<<< HEAD
alt.Chart(enforcement_actions_2023).mark_line().encode(
...
# Example: OIG_df contains 'Title', 'Category', 'Date' columns
# OIG_df = pd.read_csv("your_data.csv")  # Load your data

# Step 1: Classify the enforcement actions into five topics based on Title
def classify_topic(title):
    title = title.lower()
    if 'health care' in title or 'medicare' in title or 'medicaid' in title:
        return 'Health Care Fraud'
    elif 'financial' in title or 'bank' in title or 'fraud' in title:
...
# Apply the classification function to the 'Title' column to create a new 'Topic' column
OIG_df['Topic'] = OIG_df['Title'].apply(classify_topic)

# Step 2: Add a new 'Is Criminal and Civil Actions' column to differentiate categories
OIG_df['Is Criminal and Civil Actions'] = OIG_df['Category'].apply(
    lambda x: True if 'Criminal and Civil Actions' in x else False)

# Step 3: Group by date and category/topic
# First, make sure 'Date' is a datetime object
OIG_df['Date'] = pd.to_datetime(OIG_df['Date'])

# Group by date, category, and topic to count occurrences
category_counts = OIG_df.groupby([OIG_df['Date'].dt.to_period('M'), 'Category']).size().reset_index(name='Count')

# Group by date and topic (for Criminal and Civil Actions)
topic_counts = OIG_df[OIG_df['Category'] == 'Criminal and Civil Actions'].groupby([OIG_df['Date'].dt.to_period('M'), 'Topic']).size().reset_index(name='Count')

# Step 4: Plotting with Altair
# Combine both category_counts and topic_counts into one dataframe
category_counts['Type'] = 'Category'
topic_counts['Type'] = 'Topic'

# Concatenate the dataframes
plot_data = pd.concat([category_counts, topic_counts], ignore_index=True)

# Altair Plot
chart = alt.Chart(plot_data).mark_line().encode(
    x='Date:T',  # Time on the x-axis
    y='Count:Q',  # Count of enforcement actions on the y-axis
    color='Type:N',  # Color by 'Type' (Category or Topic)
    detail='Type:N',  # Separate lines for each type
    strokeDash='Type:N',  # Different line styles (solid vs. dashed)
    tooltip=['Date:T', 'Count:Q', 'Type:N']
).facet(
    column='Type:N',  # Facet by 'Type' (Category vs. Topic)
    header=alt.Header(title='Enforcement Actions by Category and Topic')
).properties(
    title='Number of Enforcement Actions Over Time',
    width=400,
    height=300
).configure_axis(
    labelAngle=-45  # Rotate the x-axis labels for better readability
)
...
def assign_topic(title):
    title = title.lower()
    if 'hospital' in title or 'health' in title:
        return 'Health Care Fraud'
    elif 'bank' in title or 'financial' in title:
        return 'Financial Fraud'
    elif 'drug' in title or 'pharmaceutical' in title:
        return 'Drug Enforcement'
    elif 'bribe' in title or 'corrupt' in title:
        return 'Bribery/Corruption'
    else:
        return 'Other'
