...
OIG_df['Date'] = pd.to_datetime(OIG_df['Date'], errors='coerce')

category_filter = OIG_df[OIG_df['Category'].isin(['Criminal and Civil Actions', 'State Enforcement Agencies'])]
...
# Assuming category_counts is already prepared as shown in previous steps

# Create the line chart with Altair
line_chart = alt.Chart(category_counts).mark_line().encode(
    x='Date:T',  # X-axis: Date (Temporal)
    y='Count:Q',  # Y-axis: Count of actions (Quantitative)
    color='Category:N',  # Color split by Category (Nominal)
    tooltip=['Date:T', 'Category:N', 'Count:Q']  # Show Date, Category, and Count in tooltip
).properties(
