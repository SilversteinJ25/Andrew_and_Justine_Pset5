...
capture both year and month at once.  Asked ChatGPT how to convert an object into a datetime object. Also asked ChatGPT how to create a function that would loop in order to move forward through pages. The main change was switching to a while loop.
...
enforcement_actions_2023["Observation"] = one_dummy(len(enforcement_actions_2023))
...
category_counts = category_counts.dropna(subset=['Date'])
...
line_chart = alt.Chart(category_counts).mark_line().encode(
    ...
    title='Number of Enforcement Actions Over Time vs Number of State Agencies',
    width=400,
    height=300
).configure_axis(
    labelAngle=-45  
...
```{python}
enforcement_actions_2021['Date'] = pd.to_datetime(enforcement_actions_2021['Date'])

topics_keywords = {
    'Health Care Fraud': ['health', 'doctor', 'medicare'],
...
enforcement_actions_2021['Topic'] = enforcement_actions_2021['Title of Enforcement Action'].apply(categorize_action)

enforcement_actions_2021_filtered = enforcement_actions_2021[enforcement_actions_2021['Category'] == 'Criminal and Civil Actions']

enforcement_actions_2021_grouped = enforcement_actions_2021_filtered.groupby([enforcement_actions_2021_filtered['Date'], 'Topic']).size().reset_index(name='Count')

print(enforcement_actions_2021_grouped)
```

```{python}
enforcement_actions_2021_filtered['Year-Month'] = enforcement_actions_2021_filtered['Date'].dt.to_period('M').astype(str)

enforcement_actions_2021_grouped = enforcement_actions_2021_filtered.groupby(['Year-Month', 'Topic']).size().reset_index(name='Count')

print(enforcement_actions_2021_grouped)

line_chart = alt.Chart(enforcement_actions_2021_grouped).mark_line().encode(
    x='Year-Month:T', 
    y='Count:Q',  
    color='Topic:N', 
