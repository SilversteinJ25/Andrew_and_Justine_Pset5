...
# Step 2: Filter the dataset for 'Criminal and Civil Actions'
criminal_and_civil_df = enforcement_actions_2021[enforcement_actions_2021['Category'] == 'Criminal and Civil Actions']

# Step 3: Define a function to assign topics based on keywords in the title
...
# Step 4: Apply the function to the 'Title of Enforcement Action' column
criminal_and_civil_df['Topic'] = criminal_and_civil_df['Title of Enforcement Action'].apply(assign_topic)

# Step 5: Group by Date (monthly) and Topic, and count the number of enforcement actions
...
# Step 6: Convert the 'Date' to a timestamp for Altair
category_counts['Date'] = category_counts['Date'].dt.to_timestamp()

# Step 7: Create the Altair line chart
line_chart = alt.Chart(category_counts).mark_line().encode(
    x='Date:T',        # Date on the x-axis (temporal)
    y='Count:Q',       # Count of enforcement actions on the y-axis (quantitative)
    color='Topic:N',   # Different colors for each topic
    tooltip=['Date:T', 'Topic:N', 'Count:Q']  # Tooltip to display values on hover
).properties(
